...
class DatasetManager(QMainWindow):
    ...
    def init_ui(self):
        ...
        self.dest_in = QLineEdit(); self.dest_in.setPlaceholderText("Dest folder (auto if empty)")
        btn_search = QPushButton("Search/Filter Table"); btn_search.clicked.connect(self.perform_dry_run_search)
        ...
        r3.addWidget(btn_search); r3.addWidget(btn_organize)
        
        tl.addLayout(r1); tl.addLayout(r2); tl.addLayout(r3)
        ...
        self.analysis_filter = QLineEdit(); self.analysis_filter.setPlaceholderText("Filter tags...")
        self.analysis_filter.textChanged.connect(self.filter_analysis_table)
        
        btn_dedupe = QPushButton("Deduplicate Dataset")
        btn_dedupe.setStyleSheet("background-color: #c0392b; color: white;")
        btn_dedupe.clicked.connect(self.run_deduplication)

        ctrl_layout.addWidget(btn_analyze)
        ctrl_layout.addWidget(self.analysis_filter)
        ctrl_layout.addWidget(btn_dedupe)
    ...
    def go_up_level(self):
        ...
        if self.root_dir and not parent.startswith(self.root_dir) and parent != self.root_dir: return
        ...
        if os.path.isdir(path):
            if self.current_dir and path != self.current_dir:
                try:
                    ...
                    self.dest_in.setText(rel if not rel.startswith("..") else path)
                except ValueError: self.dest_in.setText(path)
            self.load_folder_contents(path)
    ...
    def load_folder_contents(self, path):
        ...
        try:
            ...
            for i, f in enumerate(files):
                ...
                # Cache hash in row data for duplicate-aware editing
                item_name.setData(Qt.UserRole + 1, get_image_hash(full_img_path))
    ...
    def run_deduplication(self):
        ...
        # Strategy selection
        msg = QMessageBox()
        msg.setWindowTitle("Deduplication Strategy")
        msg.setText("Duplicates found will be removed. Which occurrence should be kept?")
        top_btn = msg.addButton("Keep Upper (Root-most)", QMessageBox.ActionRole)
        bot_btn = msg.addButton("Keep Lower (Deepest)", QMessageBox.ActionRole)
        msg.addButton(QMessageBox.Cancel)
        msg.exec()
        
        if msg.clickedButton() not in [top_btn, bot_btn]: return
        keep_upper = (msg.clickedButton() == top_btn)
        ...
        for h, paths in hashes.items():
            if len(paths) > 1:
                # Sort by path depth
                paths.sort(key=lambda p: p.count(os.sep), reverse=not keep_upper)
                keep_path = paths[0]
                for to_del in paths[1:]:
                    try:
                        os.remove(to_del)
                        txt = os.path.splitext(to_del)[0] + ".txt"
                        if os.path.exists(txt): os.remove(txt)
                        deleted += 1
                    except: pass
        
        QMessageBox.information(self, "Deduplication Done", f"Removed {deleted} duplicate image/tag pairs.")
        self.load_folder_contents(self.current_dir)

    def get_rows_by_hash(self, target_hash):
        """Find all table rows matching a specific hash for synchronized editing."""
        if not target_hash: return []
        rows = []
        for r in range(self.table.rowCount()):
            if self.table.item(r, 0).data(Qt.UserRole + 1) == target_hash:
                rows.append(r)
        return rows
    ...
    def run_global_analysis(self):
        ...
        for root, _, files in os.walk(self.root_dir):
            for f in files:
                if f.lower().endswith(('.png', '.jpg', '.jpeg', '.webp')):
                    path = os.path.join(root, f)
                    h = get_image_hash(path)
                    # Deduplication: only count tags from 1 occurrence of a hash
                    ...
                    if os.path.exists(txt):
                        try:
                            with open(txt, 'r', encoding='utf-8', errors='ignore') as fh:
                                tags = [t.strip() for t in fh.read().split(",") if t.strip()]
        ...
        for i, (tag, count) in enumerate(tag_counts.items()):
            ...
            c_item = QTableWidgetItem(); c_item.setData(Qt.EditRole, count)
            self.analysis_table.setItem(i, 1, c_item)
        self.analysis_table.setSortingEnabled(True)
        self.analysis_table.sortByColumn(1, Qt.DescendingOrder)
        self.analysis_stats.setText(f"Unique Images (by hash): {file_count} | Unique Tags: {len(tag_counts)}")
    ...
    def perform_dry_run_search(self):
        ...
        scan_path = self.root_dir if scope == "Global Dataset" else self.current_dir
        ...
        for i, (root, f, tags) in enumerate(matches):
            ...
            item.setData(Qt.UserRole + 1, get_image_hash(os.path.join(root, f)))
            self.table.setItem(i, 0, item)
            self.table.setItem(i, 1, QTableWidgetItem(tags))
        self.table.resizeRowsToContents()

    # --- BATCH TAG UTILS (DUPLICATE-AWARE) ---
    def _apply_batch_logic(self, mode):
        # We track modified hashes to ensure we edit all visual occurrences of the same image
        f, r_val = self.f_in.text().strip(), self.r_in.text().strip()
        add_val = self.batch_in.text().strip()
        
        processed_hashes = set()
        for i in range(self.table.rowCount()):
            h = self.table.item(i, 0).data(Qt.UserRole + 1)
            # Find all rows in current view sharing this hash
            peer_rows = self.get_rows_by_hash(h)
            
            for row_idx in peer_rows:
                it = self.table.item(row_idx, 1)
                cur_tags = [t.strip() for t in it.text().split(",") if t.strip()]
                
                if mode == "replace" and f:
                    new_tags = [r_val if t == f else t for t in cur_tags]
                    it.setText(", ".join([t for t in new_tags if t]))
                elif mode == "prepend" and add_val:
                    it.setText(f"{add_val}, {it.text()}" if it.text() else add_val)
                elif mode == "append" and add_val:
                    it.setText(f"{it.text()}, {add_val}" if it.text() and not it.text().endswith(",") else f"{it.text()} {add_val}" if it.text() else add_val)
                    
        self.table.resizeRowsToContents()

    def do_replace(self): self._apply_batch_logic("replace")
    def do_prepend(self): self._apply_batch_logic("prepend")
    def do_append(self): self._apply_batch_logic("append")
    ...
    def organize_by_tags(self):
        target_tag = self.f_in.text().strip()
        if not target_tag or not self.current_dir: return
        ...
        matches = []
        for root, _, files in (os.walk(scan_path) if scope != "Current Folder" else [(scan_path, [], os.listdir(scan_path))]):
                    ...
                    if os.path.exists(txt_path):
                        try:
                            with open(txt_path, 'r', encoding='utf-8', errors='ignore') as fh:
                                ...
                                if all(qt in file_tags for qt in query_tags): matches.append((root, f))
                        except: pass

        if matches and QMessageBox.Yes == QMessageBox.question(self, "Confirm", f"Found {len(matches)} pairs. {'Copy' if is_copy else 'Move'}?"):
            if not os.path.exists(dest_path): os.makedirs(dest_path)
            for root, f in matches:
                src_img, src_txt = os.path.join(root, f), os.path.join(root, os.path.splitext(f)[0] + ".txt")
                try:
                    if is_copy: 
                        shutil.copy2(src_img, os.path.join(dest_path, f))
                        shutil.copy2(src_txt, os.path.join(dest_path, os.path.basename(src_txt)))
                    else:
                        shutil.move(src_img, os.path.join(dest_path, f))
                        shutil.move(src_txt, os.path.join(dest_path, os.path.basename(src_txt)))
                except: pass
            self.load_folder_contents(self.current_dir)

    def on_analysis_double_click(self, index):
        tag = self.analysis_table.item(index.row(), 0).text().strip()
        cur = self.f_in.text().strip()
        self.f_in.setText(f"{cur}, {tag}" if cur and tag not in cur else tag if not cur else cur)
        ...
        for r in range(self.table.rowCount()):
            ...
            if folder:
                with open(os.path.join(folder, os.path.splitext(fn)[0] + ".txt"), 'w', encoding='utf-8') as f:
                    f.write(txt)
        QMessageBox.information(self, "Saved", "All modifications committed.")

    def initiate_drag(self, supportedActions):
        selected_rows = self.table.selectionModel().selectedRows()
        if not selected_rows: return
        filenames = [self.table.item(r.row(), 0).text() for r in selected_rows]
        mime_data = QMimeData(); mime_data.setText(",".join(filenames))
        drag = QDrag(self); drag.setMimeData(mime_data)
        if drag.exec(Qt.MoveAction) == Qt.MoveAction: self.load_folder_contents(self.current_dir)
        ...
        if os.path.isdir(target_dir):
            ...
            for f in filenames:
                ...
                try:
                    ...
                    if os.path.exists(src_txt): shutil.move(src_txt, os.path.join(target_dir, os.path.basename(src_txt)))
                except: pass
            self.load_folder_contents(self.current_dir)
