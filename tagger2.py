    ...
    def run(self):
        try:
            ...
            for cfg in self.model_configs:
                ...
                self.diag.emit(f"Loading: {repo} (Sub: {subfolder})")
                ...
                if "cl_tagger" in repo:
                    ...
                    with open(tags_path, 'r', encoding='utf-8') as f:
                        mapping = json.load(f)
                    tag_names = [mapping[idx]['tag'] for idx in sorted(mapping.keys(), key=int)]
                    tag_categories = [mapping[idx]['category'] for idx in sorted(mapping.keys(), key=int)]
                else:
                ...
                # Dynamic shape handling for symbolic dimensions (e.g. 'height', 'width')
                def get_dim(val):
                    try:
                        return int(val)
                    except (ValueError, TypeError):
                        return 448 # Fallback for symbolic dims

                # Determine Layout: NHWC vs NCHW
                is_nchw = False
                if len(shape) >= 4:
                    ...
                    h, w = 448, 448 # Default fallback

                self.diag.emit(f"Model {repo} initialized. Shape: {shape} (Final: {w}x{h}, NCHW: {is_nchw})")
            ...
            for i, filename in enumerate(files):
                ...
                for s in sessions:
                    ...
                    if s["is_cl"]:
                        # CL models expect normalized 0-1 and NCHW
                        img_proc /= 255.0
                    ...
                    for idx, score in enumerate(probs):
                        if score >= s["thresh"]:
                            tag = s["tags"][idx]
                            cat = str(s["cats"][idx]).lower()
                            
                            keep = False
                            if s["is_cl"]:
                                if cat == "general" and self.filters['general']: keep = True
                                elif cat == "character" and self.filters['character']: keep = True
                                elif cat == "copyright" and self.filters['copyright']: keep = True
                                elif cat == "rating" and self.filters['rating']: keep = True
                                elif cat == "meta" and self.filters['meta']: keep = True
                                elif cat == "quality" and self.filters['quality']: keep = True
                                elif cat == "model" and self.filters['model']: keep = True
                            else:
                                cat_int = int(s["cats"][idx])
                                if cat_int == 0 and self.filters['general']: keep = True
                                elif cat_int == 4 and self.filters['character']: keep = True
                                elif cat_int == 1 and self.filters['copyright']: keep = True
                                elif cat_int == 5 and self.filters['meta']: keep = True
                                elif cat_int == 9 and self.filters['rating']: keep = True
...
class App(QMainWindow):
    def __init__(self):
        super().__init__()
        self.setWindowTitle("Booru Tagger v2026.1")
    ...
    def init_ui(self):
        ...
        for name, data in MODELS.items():
            ...
            self.model_refs[name] = {"chk": chk, "spn": spn, "repo": data['repo'], "subfolder": data['subfolder']}
        ...
        self.f_checks = {
            ...
            "meta": QCheckBox("Meta (Resolution, etc)"),
            "quality": QCheckBox("Quality (CL Only)"),
            "model": QCheckBox("Model Specs (CL Only)")
        }
